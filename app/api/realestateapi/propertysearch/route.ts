...
export async function POST(request: Request) {
  try {
    ...
    const { city, searchType, state, title } = body;

    if (!city || !searchType || !state || !title) {
      return NextResponse.json({ message: 'Missing required fields: city, searchType, state, title' }, { status: 400 });
    }
